...
function plugin:access(plugin_conf)
  ...
  local country = kong.request.get_header("X-Country")
  local regione = kong.request.get_header("X-Regione")

  if not (country == "Italy" and regione) then
    kong.log("No cluster redirect is needed")
    return
  end 
  ...
  local ok, err = kong.service.set_upstream(redirect)
  if not ok then
    kong.log.err(err)
    return
  end
